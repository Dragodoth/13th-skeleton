...
var handlers = {
  ...
  Literal(node, state) {
    const value = node.raw ?? (typeof node.value === "string" ? JSON.stringify(node.value) : String(node.value));
  ...
  ObjectExpression(node, state) {
    state.commands.push("{");
    sequence(node.properties, state, true, (p, state2) => {
  ...
  ReturnStatement(node, state) {
    if (node.argument) {
      const contains_comment = node.argument.leadingComments && node.argument.leadingComments.some((comment) => comment.type === "Line");
...
//# sourceMappingURL=chunk-FKDVJUZP.js.map
