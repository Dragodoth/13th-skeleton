...
function Parser($$anchor, $$props) {
  ...
  if_block(
    ...
    ($$anchor2) => {
      ...
      if_block(node_3, () => renderers()[type()], ($$anchor3) => {
        var fragment_4 = comment();
        var node_4 = first_child(fragment_4);
        if_block(
          node_4,
          () => strict_equals(type(), "table"),
          ($$anchor4) => {
                          ...
                          $$component3($$anchor9, {
                            children: wrap_snippet(Parser, ($$anchor10, $$slotProps3) => {
                              ...
                              each(node_8, 1, header, index, ($$anchor11, headerItem, i) => {
                                ...
                                var align = derived_safe_equal(() => $$restProps.align[i] || "center");
                            ...
                            $$component3($$anchor10, {
                              children: wrap_snippet(Parser, ($$anchor11, $$slotProps3) => {
                                ...
                                each(node_14, 1, () => get(row), index, ($$anchor12, cells, i) => {
                                  ...
                                  var align_1 = derived_safe_equal(() => $$restProps.align[i] || "center");
    ...
    args.forEach((pack) => {
      ...
      if (pack.renderer) {
        ...
        for (const prop2 in pack.renderer) {
          const prevRenderer = renderer[prop2];
          renderer[prop2] = (...args2) => {
            let ret = pack.renderer[prop2].apply(renderer, args2);
            if (ret === false) {
      ...
      if (pack.tokenizer) {
        ...
        for (const prop2 in pack.tokenizer) {
          const prevTokenizer = tokenizer[prop2];
          tokenizer[prop2] = (...args2) => {
            let ret = pack.tokenizer[prop2].apply(tokenizer, args2);
            if (ret === false) {
      ...
      if (pack.hooks) {
        ...
        for (const prop2 in pack.hooks) {
          const prevHook = hooks[prop2];
          if (Hooks.passThroughHooks.has(prop2)) {
            hooks[prop2] = (arg) => {
              if (this.defaults.async) {
                return Promise.resolve(pack.hooks[prop2].call(hooks, arg)).then((ret2) => {
                  return prevHook.call(hooks, ret2);
                });
              }
              const ret = pack.hooks[prop2].call(hooks, arg);
              return prevHook.call(hooks, ret);
            };
          } else {
            hooks[prop2] = (...args2) => {
              let ret = pack.hooks[prop2].apply(hooks, args2);
              if (ret === false) {
...
function Heading($$anchor, $$props) {
  ...
  legacy_pre_effect(() => deep_read_state(text2()), () => {
    set(id, options2.headerIds ? options2.headerPrefix + slug(text2()) : void 0);
  });
  legacy_pre_effect_reset();
  init();
  var fragment = comment();
...
function Codespan($$anchor, $$props) {
  ...
  template_effect(() => set_text(text2, raw().replace(/`/g, "")));
  reset(code);
  append($$anchor, code);
  return pop({ ...legacy_api() });
}
...
function SvelteMarkdown($$anchor, $$props) {
  ...
  legacy_pre_effect(
    () => (defaultOptions, deep_read_state(options2())),
    () => {
      set(combinedOptions, { ...defaultOptions, ...options2() });
    }
  );
  legacy_pre_effect(
    () => (get(preprocessed), get(tokens), deep_read_state(source()), get(lexer2), Lexer, get(combinedOptions), deep_read_state(isInline())),
    () => {
      if (get(preprocessed)) {
        set(tokens, source());
      } else {
        set(lexer2, new Lexer(get(combinedOptions)));
        set(tokens, isInline() ? get(lexer2).inlineTokens(source()) : get(lexer2).lex(source()));
